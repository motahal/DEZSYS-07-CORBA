...
public class Client extends CallBackPOA {
	public static void main(String[] args)  {
		...
		try {
			...
			CallBack myCallback = CallBackHelper.narrow (root_poa.servant_to_reference (new Client()));

			/* Ausfuehren */
			echo.one_time(myCallback, "einmalige Nachricht");
			short period_secs = 1;
			echo.register(myCallback, "register -> jede sekunde", period_secs);

			/* Dem Callback Zeit geben */
			try {
				Thread.sleep(6000);
			} catch (Exception e) {
				e.printStackTrace();
